...
class CacheResponse {
  ...
  int _cacheResponseAge() {
    ...
    final ageSeconds = int.parse(headers[ageHeader]?.firstOrNull ?? '-1');
  ...
  static Future<CacheResponse> fromResponse({
    ...
    final dateStr = response.headers[dateHeader]?.firstOrNull;
    DateTime? date;
    if (dateStr != null) {
      try {
    ...
    final expiresDateStr = response.headers[expiresHeader]?.firstOrNull;
    ...
    return CacheResponse(
      ...
      content: null,
      date: date,
      eTag: response.headers[etagHeader]?.firstOrNull,
      expires: httpExpiresDate,
      headers: null,
      key: key,
      lastModified: response.headers[lastModifiedHeader]?.firstOrNull,
